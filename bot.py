...
def ai_translate(text, target_instruction):
    """Gemini AI ကို သုံး၍ အလွန်တိကျစွာ ဘာသာပြန်ခြင်း"""
    try:
        prompt = f"Translate the following text. {target_instruction}\nText: {text}\nProvide only the translated text as output without any preamble, explanation, or quotes."
...
async def translate_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_myanmar:
            ...
            raw_spanish = ai_translate(text, "Translate to polite professional Venezuelan Spanish manager style (using 'Usted').")
